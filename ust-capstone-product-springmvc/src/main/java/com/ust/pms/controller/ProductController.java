...
public class ProductController {
	...
	/*
	 * @RequestMapping("/deleteProductById") public ModelAndView
	 * deleteProductById(Product product) {
	 * 
	 * ModelAndView mav = new ModelAndView();
	 * mav.setViewName("searchProductByIdForm"); mav.addObject("command", new
	 * Product()); int productId = product.getProductId();
	 * 
	 * if (productService.isProductExist(productId)) {
	 * productService.deleteProductById(productId); mav.addObject("message",
	 * "product with product id" + productId + " deleted successfully");
	 * 
	 * } else {
	 * 
	 * mav.addObject("message", "Product with product id :" + productId +
	 * " doesn't exist"); // return new
	 * ModelAndView("searchProductByIdForm","command",new Product());
	 * 
	 * } return mav;
	 * 
	 * }
